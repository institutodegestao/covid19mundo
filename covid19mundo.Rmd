...
italia$ctrl1 <- ifelse(italia$date <= tagFin$date, italia$casosMM7, 1)

j <- tagMaxMin
i <- 1
ctrl2 <- c()
while (i <= as.integer(tagDiasUp)+1) {
  if(j >= tagVar){
  ctrl2[i] = j - tagVar 
  j = j - tagVar}
  else
...
italia$ctrl <- ifelse(italia$ctrl1 != 1, italia$ctrl1, as.integer(ctrl2))
```
